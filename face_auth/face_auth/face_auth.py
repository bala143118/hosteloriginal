...
class AuthResult:
    ...
    def authorization_label(self) -> str:
        """User-facing verification status for overlays and UI."""
        return "Authorized" if self.matched else "Unauthorized"
...
class FaceAuthSystem:
    ...
    def draw_results(
        ...
        Draw bounding boxes + labels onto a copy of `frame`:
            * green box + "Authorized" + confidence for matched faces
            * red box + "Unauthorized" + confidence for unmatched faces
        ...
        for result in results:
            ...
            label = f"{result.authorization_label} ({result.confidence * 100:.1f}%)"
